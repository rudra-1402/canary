...
describe('seed invariants (Python generator → Node app contract)', () => {
  ...
  describe('role correctness — verified clean 2026-07-29', () => {
    ...
    it(
      ...
      async () => {
        expect(await countRoleMismatch('engagements', 'freelancerProfileId', 'freelancer')).toBe(0);
      ...
      const stale = await db()
        ...
        .aggregate(
          [
            ...
            {
              $lookup: { from: 'jobposts', localField: 'jobPostId', foreignField: '_id', as: 'j' },
            },
            { $unwind: '$j' },
            { $match: { 'j.status': 'open' } },
            { $count: 'n' },
  ...
  it.fails(
    ...
    async () => {
      for (const c of ['jobposts', 'proposals', 'engagements']) {
        expect((await distinctStatuses(c)).length, `${c} only ever has one status`).toBeGreaterThan(
          1,
        );
      }
    },
